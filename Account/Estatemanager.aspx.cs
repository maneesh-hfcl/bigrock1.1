...
public partial class Account_Login : Page
{
   // MySqlConnection con = new MySqlConnection(@"Data source=localhost;port=3306;Initial catalog=DJB;User ID=root;Password=root");
    ...
    protected void CreateUser_Click(object sender, EventArgs e)
    {
        ...
        cmd.CommandText = "select * from estatelogin where Username='"+UserName.Text+"'AND Password1='"+Password.Text+"'";
        cmd.ExecuteNonQuery();
        DataTable dt = new DataTable();
        MySqlDataAdapter da = new MySqlDataAdapter(cmd);
        da.Fill(dt);
          foreach (DataRow dr in dt.Rows) {
            Session["Eusername"] = dr["username"].ToString();
            con.Close();
            Response.Redirect("~/Account/EstateM.aspx");
        }
